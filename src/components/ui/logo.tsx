...
export function Logo({
  ...
  const sizeMap = {
    ...
    xl: {
      box: 'h-24 sm:h-28 w-auto',
      imgClass: 'h-24 sm:h-28 w-auto object-contain',
    },
  };
    ...
    <div className={`inline-flex items-center ${className}`}>
      <div className={`relative ${currentSize.box} shrink-0 flex items-center justify-center`}>
        ...
        <img
          src="/logo.png"
          alt="Tá Marcado"
          className={`${currentSize.imgClass} dark:bg-white dark:rounded-xl dark:px-2.5 dark:py-1 transition-all`}
        />
      </div>
    </div>
  );
